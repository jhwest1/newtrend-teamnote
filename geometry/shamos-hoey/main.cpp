...
namespace shamos_hoey {
	...
	bool operator<(const line &l, const int &y) { 
		return ccw(l.s, l.e, point{sweep, y}) > 0; 
	}
	bool operator<(const int &y, const line &l) {
		return ccw(l.s, l.e, point{sweep, y}) < 0;
	}
	bool isect(const line &l, const line &r) {
		return line_intersect(l.s, l.e, r.s, r.e);
	}

	struct event {
		int x, type, idx;
		bool operator<(const event &ot) const {
			return x == ot.x ? type < ot.type : x < ot.x;
		}
	};
	vector<line> L;
	vector<event> E;
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		point s, e;
		cin >> s.x >> s.y >> e.x >> e.y;
		shamos_hoey::add_line(s, e);
	}
	cout << shamos_hoey::calc() << '\n';
}