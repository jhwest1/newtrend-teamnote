...
void add_edge(int ui, int vi, T wi) { g[ui][vi].w = g[vi][ui].w = wi; }
...
int main()
{
	...
	for(int i=1; i<=M; i++)
	{
		...
		u++; v++; w*=X;
		V.push_back({u, v, w});
		add_edge(rand()%N+1, rand()%N+1, w);
	}
	solve();
	init(N);
	for(auto [u, v, w] : V) add_edge(u, v, w);
	auto [a, b] = solve();
	cout << b << " " << a/X << "\n";
	for(int i=1; i<=N; i++) if(match[i] && match[i]<i) cout << i-1 << " " << match[i]-1 << "\n";
}