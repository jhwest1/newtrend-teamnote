...
// 1-based
// MUST call init() before calling calc()
...
namespace dominator_tree {
	...
	int S[SV], I[SV];
	vector<int> g[SV], h[SV], V[SV];

	void init(int _n) { 
		n = _n; sp = 0;
		for (int i = 1; i <= n; i++) dfn[i] = 0, ord[i] = 0, S[i] = 1e9, I[i] = 0;
	}
