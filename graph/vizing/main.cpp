...
// 1-based
// Given a bipartite graph with maximum degree D,
// it finds a D-edge-coloring in O(NM) time

namespace Vizing{ // returns edge coloring in adjacent matrix G. 1 - based
	int C[MAXN+10][MAXN+10], G[MAXN+10][MAXN+10];
	void clear(int N){
		for(int i=0; i<=N; i++){
			for(int j=0; j<=N; j++) C[i][j] = G[i][j] = 0;
		}
	}
	// edge list (simple graph)
	void solve(vector<pii> &E, int N, int M){
		...
		for(int t = 0; t < E.size(); t++){
			...
			while(!G[u][v0]){
				...
				if(!C[v][c]) for(a = (int)L.size()-1; a >= 0; a--)
